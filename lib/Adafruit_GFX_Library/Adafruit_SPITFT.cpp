...
*/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t mosi,
...
*/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, SPIClass * spiClass, int8_t cs, int8_t dc,
                                 int8_t rst)
    : Adafruit_GFX(w, h), connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc) {
    hwspi._spi = spiClass;
#if defined(USE_FAST_PINIO)
...
*/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, tftBusWidth busWidth, int8_t d0, int8_t wr,
    ...
    if (((connection == TFT_HARD_SPI) || (connection == TFT_PARALLEL)) &&
        ...
        if (dptr = dma.addDescriptor(NULL, NULL, 42, DMA_BEAT_SIZE_BYTE, false, false)) {
            ...
            if ((pixelBuf[0] = (uint16_t *)malloc(maxFillLen * sizeof(uint16_t)))) {
                ...
                // is, so here we are.
                if ((descriptor =
                         (DmacDescriptor *)memalign(16, numDescriptors * sizeof(DmacDescriptor)))) {
                    int dmac_id;
...
*/
inline void Adafruit_SPITFT::writeFillRectPreclipped(int16_t x, int16_t y, int16_t w, int16_t h,
                                                     uint16_t color) {
...
*/
void Adafruit_SPITFT::drawRGBBitmap(int16_t x, int16_t y, uint16_t * pcolors, int16_t w,
                                    int16_t h) {
...
/*!
 ...
 @param  numDataBytes  The number of bytes we should send
 */
void Adafruit_SPITFT::sendCommand16(uint16_t commandWord, const uint8_t * dataBytes,
                                    uint8_t numDataBytes) {
    SPI_BEGIN_TRANSACTION();
    if (_cs >= 0)
        SPI_CS_LOW();
